...
set -euo pipefail
cd "$(dirname "$0")"
python3 - "${1:-output/masterappsurvey-survey-review-20260918.zip}" <<'PY'
...
assert manifest['version'] == '1.3.3', 'Review baseline changed: verify release version before packaging.'
...
print(json.dumps({'archive':str(output),'bytes':output.stat().st_size,'sha256':hashlib.sha256(output.read_bytes()).hexdigest(),'runtime_files':len(files),'manifest_version':manifest['version'],'status':'LOCAL REVIEW ONLY; no upload, publication or installation'},indent=2))
PY
