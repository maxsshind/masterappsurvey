...
const fields=['clear_height_ft','office_sf','lease_area','year_built','loading',...flags];
...
const labels={clear_height_ft:'Clear height (ft)',office_sf:'Office SF',lease_area:'Available lease SF',year_built:'Year built',loading:'Loading',class_a:'Class A',heavy_power:'Heavy power',has_rail:'Has rail',has_truckwell_or_dock:'Has truckwell or dock'};
...
function parse(field,value){
 ...
 if(field==='loading')return typeof value==='string'&&value.trim().length<=4000?{value:value.trim()||null}:{value:null,error:'Loading must be text up to 4,000 characters.'};
...
function serialize(raw){const values={},issues=[];for(const f of fields){const p=parse(f,raw[f]);values[f]=p.value;if(p.error)issues.push({field:f,message:p.error});}return {values,issues};}
...
function fromScrape(d){
 const f=d.selectedSpace?.availableRange?{}:d.propertyFacts||{},values={},unresolved=[];
 if(d.selectedSpace?.availableRange)unresolved.push('Divisible space: confirm office and loading for the offered portion.');
 for(const [field,raw] of [['clear_height_ft',f.clearHeight],['office_sf',f.officeSf],['lease_area',d.selectedSpace?.availableRange?null:d.selectedSpace?.availableSf],['year_built',d.yearBuilt]]){
  if(raw==null||raw==='')continue;const p=parse(field,raw);if(p.error)unresolved.push(`${labels[field]}: ${raw}`);else values[field]=p.value;
 }
 if(f.loading)values.loading=f.loading;
 ...
 if(f.power)unresolved.push(`Power: ${f.power}`);
 if(f.railLine&&!Object.hasOwn(values,'has_rail'))unresolved.push(`Rail source: ${f.railLine}`);
 return {values,unresolved};
}
return {fields,flags,labels,propertyTypes,normalizePropertyTypes,height,area,parse,serialize,fromScrape};
});
